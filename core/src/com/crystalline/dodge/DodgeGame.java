...
public class DodgeGame implements ApplicationListener
{
	...
	int calcFontSize()
	{
		return (int)(cam.viewportWidth/20f);
	}
	...
	public void create()
	{
		...
		pVel = new Vector2();
		FreeTypeFontGenerator generator = new FreeTypeFontGenerator(Gdx.files.internal("Lato-Light.ttf"));

		FreeTypeFontParameter parameter = new FreeTypeFontParameter();
		parameter.size = calcFontSize();
	...
	private void printCenter(float y, String text)
	{
		...
		float size = glyphLayout.width;
		font.draw(batch, glyphLayout, cam.position.x-size/2f,y + glyphLayout.height/2);
	}
